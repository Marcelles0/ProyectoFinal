...
export default function Home (){
 ...
 radius = width/2.5;
	...
	document.querySelector(document).ready(function(){
		var radius = 200;
		var fields = document.querySelector(`.${styleHome.itemDot}`);
		var container = document.querySelector(`.${styleHome.dotCircle}`);
		var width = container.width();
 radius = width/2.5;
		...
		var angle = 0, step = (2*Math.PI) / fields.length;
		fields.each(function() {
			var x = Math.round(width/2 + radius * Math.cos(angle) - document.querySelector(this).width()/2);
			var y = Math.round(height/2 + radius * Math.sin(angle) - document.querySelector(this).height()/2);
		...
		document.querySelector(`.${styleHome.itemDot}`).click(function(){
			
			var dataTab= document.querySelector(this).data("tab");
			document.querySelector(`.${styleHome.itemDot}`).classList.remove(`${styleHome.active}`);
			document.querySelector(this).classList.add(`${styleHome.active}`);
			document.querySelector(`.${styleHome.CirItem}`).classList.remove(`${styleHome.active}`);
			document.querySelector( `.${styleHome.CirItem}`+ dataTab).classList.add(`${styleHome.active}`);
			i=dataTab;
			
			document.querySelector(`.${styleHome.dotCircle}`).css({
				"transform":"rotate("+(360-(i-1)*36)+"deg)",
				"transition":"2s"
			});
			document.querySelector(`.${styleHome.itemDot}`).css({
				"transform":"rotate("+((i-1)*36)+"deg)",
				"transition":"1s"
			});
		...
		setInterval(function(){
			var dataTab= document.querySelector(`.${styleHome.itemDot.active}`).data("tab");
			if(dataTab>6||i>6){
			dataTab=1;
			i=1;
			}
			document.querySelector(`.${styleHome.itemDot}`).classList.remove(`${styleHome.active}`);
			document.querySelector('[data-tab="'+i+'"]').classList.add(`${styleHome.active}`);
			document.querySelector(`.${styleHome.CirItem}`).classList.remove(`${styleHome.active}`);
			document.querySelector( `.${styleHome.CirItem}`+i).classList.add(`${styleHome.active}`);
			i++;
